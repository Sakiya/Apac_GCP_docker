...
$(document).ready(function(){
    for(var o=(new Date).getFullYear();o>1900;o--){
        if( o != '' ){
...
function dropifyFiles(){
    function e() {
        $.each($(".img-wrapper img"), function(e, t) {
            var i = $(t).attr("src");
            $(t).parent().css("background", "url(" + i + ") no-repeat center center").css("background-size", "cover");
            $(t).remove();
        });
    }
    var t = {
            ...
            if (a && a[0]) {
                ...
                s.onload = function(a) {
                    var s = $(t).parents(".upload").siblings(".preview"),
                        r = $(s).find("img");
                    if (r.length == 0) {
                        $(s).html('<img src="' + a.target.result + '" alt=""/>');
                    } else {
                        r.attr("src", a.target.result);
                    }
                    n.val(i);
                    e();
                };

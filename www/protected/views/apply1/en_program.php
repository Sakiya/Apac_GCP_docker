        ...
        <div class="right">
			...
			<?php //echo CHtml::beginForm('/' . Yii::app()->language . '/fill/apply1_program/', 'post', array('role'=>'form','name'=>'myForm','id'=>'apply3' ,'enableAjaxValidation'=>false, 'novalidate'=>true, 'class'=>'applyTable')) ; ?>
				<?php $form=$this->beginWidget('CActiveForm', array(
                    ...
                    <div class="title">Select Sector/ Room Type</div>
                    <h5>Note: The following is only made as reservation. No payment needed at this time.</h5>
                    <div class="step">Step1 <span>Please select the sector (s) you would like to partcipate in.</span></div>
                    <div class="step1-container">
	                    <?php 
		                    foreach ($Methodm1 as $i => $row){
	                    ?>
                        <div class="greenbox">
                            <h2><?=$row->MethodM1_title_en;?></h2>
                            <h6><?=$row->MethodM1_script_en;?></h6>
                            <div class="checkbox checkbox-custom"><input id="program<?=$row->MethodM1_no;?>" name="program[program<?=$row->MethodM1_no;?>]" class="program<?=$i + 1;?>" type="checkbox" ng-model="program<?=$i + 1;?>" value="true" box="item<?=$i + 1;?>" ><label></label></div>
                        </div>
                        <?php }?>
                    </div>
					<div class="parsley-errors-list filled text-center">
						<span class="error validationerror" ng-show="!program1&&!program2&&!program3">This field is required</span>
					</div>
                    <div class="step">Step2<span>Please select your room type and rank your preference from 1 to 2.</span>
                        <!--藝術無限-->
	                    <?php 
                        ...
                        <div class="step2 item<?=$i + 1;?>">
                            <div class="step2-container" ng-class="{errorshow:program<?=$i+1;?>}">
                                ...
                                <div class="big-container">
                                    ...
                                    <div class="whitebox">
                                        <h3> <?=$row->RoomM1_nm_en;?></h3>
                                        <h6><?=$row->RoomM1_size_en;?></h6>
                                        <div class="price">USD <?=number_format($row->RoomM1_price_en);?></div>
                                        <?php if (count($rows->Roomm1) > 1){?>
                                        <div class="chose">Please indicate your priority preference</div>
                                        <input type="text" method="1" name="program[art<?=$rows->MethodM1_no;?>][<?=$row->RoomM1_no;?>]" ng-model="user.item<?=$rows->MethodM1_no;?>_<?=$row->RoomM1_no;?>" max="<?=count($rows->Roomm1);?>" ng-change="changeOnPrimary('item<?=$rows->MethodM1_no;?>_<?=$row->RoomM1_no;?>','')" ng-pattern="/^[1-<?=count($rows->Roomm1);?>]{1}$/" placeholder="Unselected" ng-required="program<?=$i + 1;?>&&<?=$roomValidation;?>" />
                                        <span class="error validationerror" ng-show="myForm['program[art<?=$rows->MethodM1_no;?>][<?=$row->RoomM1_no;?>]'].$error.pattern">Please insert numbers 1-<?=count($rows->Roomm1);?></span>
										<span class="error validationerror" ng-show="myForm['program[art<?=$rows->MethodM1_no;?>][<?=$row->RoomM1_no;?>]'].$error.required && submitChecked">Please insert numbers</span>
                                        <?php }else{ ?>
                            ...
                            <div class="info"><?=nl2br($rows->MethodM1_remarker_en);?></div>
                        </div> 
                        <?php }?>
        			</div>
					<div class="btn-group">
						<a class="btn-green-border" href="<?=$this->createUrl('/fill/apply1_experience/',array('language'=>Yii::app()->language));?>">Edit last page</a>
						<!-- <button class="btn-green" type="sumbit" ng-disabled="myForm.$invalid || !program1&&!program2&&!program3">Save</button> -->
						<button class="btn-green inlin-blk error-event" type="button" ng-click="submitted(myForm.$invalid || !program1&&!program2&&!program3)">Save</button>
					</div>
    ...
    <script>
		...
		$(document).ready(function() {
		    $(".program1").change(function() {
		        $(this).prop("checked") ? $(".item1").show() : $(".item1").hide()
		    }),
